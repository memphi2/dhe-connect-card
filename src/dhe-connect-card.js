...
const isUnavailableState = (stateObj) => !stateObj || ["unknown", "unavailable", ""].includes(String(stateObj.state));
...
class DheConnectCard extends HTMLElement {
  ...
  static getConfigForm() {
    return {
      ...
      computeLabel: (schema) => FIELD_LABELS[schema.name] || undefined,
      computeHelper: (schema) => FIELD_HELPERS[schema.name] || undefined,
      assertConfig: (config) => {
  ...
  _renderRadio(entity) {
    ...
    const station = entity.stateObj?.attributes?.media_title
      || entity.stateObj?.attributes?.source
      || entity.stateObj?.attributes?.media_artist
      || "Radio";
  ...
  _renderWeather(entity) {
    ...
    const location = entity.stateObj?.attributes?.location
      || entity.stateObj?.attributes?.location_name
  ...
  _entity(key) {
    const def = ENTITY_DEFINITIONS[key];
    if (!def) return { id: "", stateObj: undefined, available: false, definition: undefined };
  ...
  _discoverEntity(def) {
    ...
    const expectedSuffixes = new Set(expectedObjectIds.map((objectId) => (
      objectId.startsWith(`${prefix}_`) ? objectId.slice(prefix.length + 1) : objectId
    )));
    const entries = Object.entries(this._hass.states)
      .filter(([entityId]) => entityId.startsWith(`${def.domain}.`));
